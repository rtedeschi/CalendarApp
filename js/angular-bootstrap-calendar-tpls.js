(function (window, angular) {
    ...
    angular.module('mwl.calendar').run([
        ...
        function ($templateCache) {
            $templateCache.put('templates/calendarMonthDay.html', '<div class="cal-month-day" ng-class="{\n            \'cal-day-outmonth\': !day.inMonth,\n            \'cal-day-inmonth\': day.inMonth,\n            \'cal-day-weekend\': day.isWeekend,\n            \'cal-day-past\': day.isPast,\n            \'cal-day-today\': day.isToday,\n            \'cal-day-future\': day.isFuture\n          }"> <div class="pull-left"><button class="btn btn-default btn-xs" ng-hide="!day.inMonth" data-toggle="tooltip" data-placement="top" title="Add a new event on this day" ng-click="dayClicked($parent.$parent.$index, $parent.$index, undefined, \'add\')"><span class="glyphicon glyphicon-plus"></span></button> <button class="btn btn-default btn-xs hidden-xs" data-toggle="tooltip" data-placement="top" title="View all events on this day" ng-hide="!day.inMonth || day.events.length == 0" ng-click="dayClicked($parent.$parent.$index, $parent.$index, undefined, \'enum\', day)"><span class="glyphicon glyphicon-eye-open"></span></button></div> <small class="cal-events-num badge badge-important hidden-xs" ng-show="eventCountBadgeTotalFilter(day.events) > 0">{{ day.events | eventCountBadgeTotal }}</small><span class="pull-right" data-cal-date ng-click="drillDown(day.label)">{{ day.label }}</span><div class="cal-day-tick" ng-show="day.isOpened"><i class="glyphicon glyphicon-chevron-up"></i> <i class="fa fa-chevron-up"></i></div><div ng-include="\'templates/calendarMonthEventsList.html\'"></div></div>');
        ...
        return {
            ...
            controller: [
                ...
                function ($scope, $sce, $timeout, moment, calendarHelper, eventCountBadgeTotalFilter) {
                    ...
                    $scope.dayClicked = function (rowIndex, cellIndex, dayClickedFirstRun, button, day) {
                        // RT
                        if (button === 'add') {
                            newEvent($scope.view[rowIndex][cellIndex].date.startOf('day').toDate());
                            return;
                        }
                        else if (button === 'enum' && day) {
                            dayEvents(day.events, $scope.view[rowIndex][cellIndex].date.startOf('day').toDate());
                            return;
                        }
                        //
