(function (window, angular) {
    ...
    angular.module('mwl.calendar').run([
        ...
        function ($templateCache) {
            $templateCache.put('templates/calendarMonthDay.html', '<div class="cal-month-day" ng-class="{\n            \'cal-day-outmonth\': !day.inMonth,\n            \'cal-day-inmonth\': day.inMonth,\n            \'cal-day-weekend\': day.isWeekend,\n            \'cal-day-past\': day.isPast,\n            \'cal-day-today\': day.isToday,\n            \'cal-day-future\': day.isFuture\n          }"><small class="cal-events-num badge badge-important hidden-xs" ng-show="eventCountBadgeTotalFilter(day.events) > 0">{{ day.events | eventCountBadgeTotal }}</small><span class="pull-right" data-cal-date ng-click="drillDown(day.label)">{{ day.label }}</span><div ng-include="\'templates/calendarMonthEventsList.html\'"></div></div>');
            //$templateCache.put('templates/calendarMonthDay.html', '<div class="cal-month-day" ng-class="{\n            \'cal-day-outmonth\': !day.inMonth,\n            \'cal-day-inmonth\': day.inMonth,\n            \'cal-day-weekend\': day.isWeekend,\n            \'cal-day-past\': day.isPast,\n            \'cal-day-today\': day.isToday,\n            \'cal-day-future\': day.isFuture\n          }"> <div class="pull-left"><button class="btn btn-default btn-xs" ng-hide="!day.inMonth" tooltip-append-to-body="true" tooltip="Add a new event on this day" ng-click="newEvent($parent.$parent.$index, $parent.$index)"><span class="glyphicon glyphicon-plus"></span></button> <button class="btn btn-default btn-xs hidden-xs" tooltip-append-to-body="true" tooltip="View all events on this day" ng-hide="!day.inMonth || day.events.length == 0" ng-click="enumEvents($parent.$parent.$index, $parent.$index, day, eventCountBadgeTotalFilter(day.events))"><span class="glyphicon glyphicon-eye-open"></span></button></div> <small class="cal-events-num badge badge-important hidden-xs" ng-show="eventCountBadgeTotalFilter(day.events) > 0">{{ day.events | eventCountBadgeTotal }}</small><span class="pull-right" data-cal-date ng-click="drillDown(day.label)">{{ day.label }}</span><div class="cal-day-tick" ng-show="day.isOpened"><i class="glyphicon glyphicon-chevron-up"></i> <i class="fa fa-chevron-up"></i></div><div ng-include="\'templates/calendarMonthEventsList.html\'"></div></div>');
            //                                                                                                                                                                                                                                                                                                                                                                                                  ^ RT                                  This button calls a new event modal                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              ^
            ...
            $templateCache.put('templates/calendarSlideBox.html', '<div class="cal-slide-box" collapse="shouldCollapse" mwl-collapse-fallback="shouldCollapse"><div class="cal-slide-content cal-event-list"><ul class="unstyled list-unstyled"><li ng-repeat="event in openEvents | orderBy: \'starts_at\'"> <span class="pull-left event" ng-class="\'event-\' + event.type" tooltip-placement="right" tooltip-append-to-body="true" tooltip="{{event.descript}}"></span> &nbsp; <a href="javascript:;" class="event-item" ng-click="eventClick({calendarEvent: event})">{{ event.title }} <span ng-show="isMonthView" ng-hide="!event.allDay">(All Day)</span> <span ng-show="isMonthView" ng-hide="event.allDay">({{ event.starts_at | date:\'shortTime\' }})</span> <span ng-show="isYearView">({{ event.starts_at | date:\'MMM d\' }})</span></a> <a href="javascript:;" class="event-item-edit" ng-if="editEventHtml && event.editable !== false" ng-bind-html="$sce.trustAsHtml(editEventHtml)" ng-click="eventEditClick({calendarEvent: event})"></a> <a href="javascript:;" class="event-item-delete" ng-if="deleteEventHtml && event.deletable !== false" ng-bind-html="$sce.trustAsHtml(deleteEventHtml)" ng-click="eventDeleteClick({calendarEvent: event})"></a></li></ul></div></div>');
            //                                                                                                                                                                                                                                                                                                                                                       ^ Tooltip shows event description, if one exists                                    ^                                                                                                                         ^ RT                                            if the event is all day, it shows "All Day" instead of an event time                                                      ^
            //$templateCache.put('templates/calendarSlideBox.html', '    <div class="cal-slide-box" collapse="shouldCollapse" mwl-collapse-fallback="shouldCollapse"><div class="cal-slide-content cal-event-list"><table class="table table-condensed"><tr ng-repeat="event in openEvents track by $index"><td><span class="event" ng-class="\'event-\' + event.type" tooltip-placement="right" tooltip-append-to-body="true" tooltip="{{event.descript}}"></span></td><td><a href="javascript:;" class="event-item" ng-click="eventClick({calendarEvent: event})">{{ event.title }}<span ng-show="isMonthView" ng-hide="!event.allDay">(All Day)</span><span ng-show="isMonthView" ng-hide="event.allDay">({{ event.starts_at | date:\'shortTime\' }})</span><span ng-show="isYearView">({{ event.starts_at | date:\'MMM d, h:mm a\' }})</span></a></td><td><a href="javascript:;" class="event-item-edit" ng-if="editEventHtml && event.editable !== false" ng-bind-html="$sce.trustAsHtml(editEventHtml)" ng-click="eventEditClick({calendarEvent: event})"></a><a href="javascript:;" class="event-item-delete" ng-if="deleteEventHtml && event.deletable !== false" ng-bind-html="$sce.trustAsHtml(deleteEventHtml)" ng-click="eventDeleteClick({calendarEvent: event})"></a></td></tr></table></div></div>');
            ...
            $templateCache.put('templates/month.html', '<div class="cal-row-fluid cal-row-head"><div class="cal-cell1" ng-repeat="day in weekDays track by $index">{{ day }}</div></div><div class="cal-month-box"><div ng-repeat="week in view track by $index"><div class="cal-row-fluid cal-before-eventlist"><div class="cal-cell1 cal-cell {{ day.highlightClass }}" ng-repeat="day in week track by $index" ng-click="enumEvents($parent.$index, $index, day, eventCountBadgeTotalFilter(day.events))" ng-class="{pointer: day.events.length > 0}"><div ng-include="\'templates/calendarMonthDay.html\'"></div></div></div><mwl-calendar-slide-box is-open="!week.isOpened"></mwl-calendar-slide-box></div></div>');
        ...
        function (moment, calendarConfig) {
            ...
            this.toggleEventBreakdown = function (view, rowIndex, cellIndex) {
                ...
                if (view[rowIndex][cellIndex].events.length > 0) {
                    ...
                    /* RT
                        Dirty workaround for button presses.
                        Because the buttons are within the day box, the
                        day box is also pressed, and the slide box is closed.
                        This prevents the slide from changing visibility
                        when a button gets pressed
                    */
                    if (!modalOpen)
                        closeAllOpenItems(); // This line was present in original code
                }
                //if (modalOpen) closeAllOpenItems(); // Closes slide box when a modal gets opened by custom buttons
                return {
                    view: view,
        ...
        return {
            ...
            controller: [
                ...
                function ($scope, $sce, $timeout, moment, calendarHelper, eventCountBadgeTotalFilter) {
                    ...
                    // RT
                    //  handling of custom buttons
                    $scope.newEvent = function (rowIndex, cellIndex) {
                        if (!modalOpen) {
                            newEvent($scope.view[rowIndex][cellIndex].date.startOf('day').toDate());
